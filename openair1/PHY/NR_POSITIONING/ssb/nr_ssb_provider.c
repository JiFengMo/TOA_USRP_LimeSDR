...
static int ssb_target_pci_enabled(const nr_toa_ue_t *ue, const nr_iq_block_t *blk)
{
  return (blk && blk->target_pci >= 0 && blk->target_pci < 1008) ||
         (ue &&
          ((ue->current_target_pci >= 0 && ue->current_target_pci < 1008) ||
           (ue->app_cfg.target_pci >= 0 && ue->app_cfg.target_pci < 1008)));
}

static int32_t ssb_active_target_pci(const nr_toa_ue_t *ue, const nr_iq_block_t *blk)
{
  if (blk && blk->target_pci >= 0 && blk->target_pci < 1008) {
    return blk->target_pci;
  }
  if (!ue) {
    return -1;
  }
  if (ue->current_target_pci >= 0 && ue->current_target_pci < 1008) {
    return ue->current_target_pci;
  }
  if (ue->app_cfg.target_pci >= 0 && ue->app_cfg.target_pci < 1008) {
    return ue->app_cfg.target_pci;
  }
  return -1;
}
                                  ...
                                  const char *tag)
{
  ...
  if (meta_fp) {
    ...
    fprintf(meta_fp, "rx_gscn=%d\n", blk->rx_gscn);
    fprintf(meta_fp, "target_pci=%d\n", blk->target_pci);
    ...
    if (sync->mib.valid) {
      fprintf(meta_fp, "mib_scs_common=%u\n", (unsigned)sync->mib.subcarrier_spacing_common);
      fprintf(meta_fp, "mib_ssb_subcarrier_offset=%u\n", (unsigned)sync->mib.ssb_subcarrier_offset);
      fprintf(meta_fp, "mib_dmrs_typeA_position=%u\n", (unsigned)sync->mib.dmrs_typeA_position);
      fprintf(meta_fp, "mib_control_resource_set_zero=%u\n", (unsigned)sync->mib.control_resource_set_zero);
      fprintf(meta_fp, "mib_search_space_zero=%u\n", (unsigned)sync->mib.search_space_zero);
      fprintf(meta_fp, "mib_cell_barred=%u\n", (unsigned)sync->mib.cell_barred);
      fprintf(meta_fp, "mib_intra_freq_reselection=%u\n", (unsigned)sync->mib.intra_freq_reselection);
    }
    fclose(meta_fp);
  }
...
static int ssb_acquire(void *ctx, const nr_iq_block_t *blk, nr_sync_state_t *sync)
{
  ...
  static uint32_t target_pbch_gate = 0;
  const float min_pss_metric = 0.15f;
  const int32_t target_pci = ssb_target_pci_enabled(ue, blk) ? ssb_active_target_pci(ue, blk) : -1;
  const uint32_t pbch_attempt_limit = (target_pci >= 0) ? 1U : 4U;
  ...
  if (target_pci >= 0 && cand_count > 0U && ((target_pbch_gate++ & 1U) != 0U)) {
    *sync = cands[0].sync;
    sync->locked = 0;
    sync->pbch_ok = 0;
                            ...
                            const nr_sync_state_t *sync, nr_toa_meas_t *meas)
{
  ...
  if (!sync->locked || !sync->mib_ok) {
    /* Once a real PBCH/MIB decode exists, allow continuous SSB tracking
     * measurements while lock is maintained. */
    return -1;
  }
